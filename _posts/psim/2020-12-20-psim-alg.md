...
between two balls. But how do I know which two
balls collided and when? Transforming this collection of equations into a pool simulator requires an
algorithm that coordinates the proper usage of these equations, and glues them together to evolve a
shot from the moment the cue ball is struck to the moment the last ball stops moving. In this post I
describe various algorithms for doing so.
...
Less impressive graphics (and music) and no slo-mo shots to scrutinize in careful detail, but having played the

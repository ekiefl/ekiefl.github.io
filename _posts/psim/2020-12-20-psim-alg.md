...
So [as mentioned](#-ball-pocket-collision), the ball-pocket collision exists as a means to determine when a ball is pocketed. Defining the ball-pocket collision condition requires a suitable geometry for pockets, which I am going to be very rudimentary about. Basically, the $j\text{th}$ pocket is going to be a cylinder with an axial center $(a^{(j)}, b^{(j)})$ and radius $R^{(j)}$. It doesn't even need an illustration but I'm perfectionist so here you go:

[![pocket_geometry]({{images}}/pocket_geometry.jpg)]({{images}}/pocket_geometry.jpg){:.center-img .width-90}

_**Figure 17**. A depiction of the cylindrical pocket geometry (shown in orange) for the $j\text{th}$ pocket. The axial center is $(a^{(j)}, b^{(j)})$ and the radius is $R^{(j)}$._

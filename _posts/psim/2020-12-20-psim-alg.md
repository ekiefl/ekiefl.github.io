...
authors: [evan]
image:
  feature: psim/psim-alg/plane_extension.jpg
  display: true
---
...
state for a given time $t$ is just the collection of individual ball states at that time $t$.
...
are never predicted, but rather detected _after_ they were already supposed to have happened.
...
really plan to model that. Instead, the ball-pocket collision is merely a way
...
is considered pocketed**. It went in the hole. Good job. To resolve the collision,
...
invalid and need to be updated to the stationary equations of motion. As a matter of interest, if the
equation was not updated, it dictates that the ball would begin spinning in the reverse direction, gaining
more and more rotational kinetic energy indefinitely. This is obviously non-physical.
...
then naturally the one that occurs in the shortest amount of time is the one that physically occurs next.
...
2. Define the minimum time until a ball-ball collision time $\Delta t_E^{\text{ball-ball}}$
2. Take the min
 yadayada
...
This means we must develop a means to calculate the time until every possible next event.
...
That means that for a given time $t$, the spinning-stationary transition event for **every spinning ball** must be
...
Alright, so we are on the search for calculating all possible next events. A
very large subcategory of next possible events is all of the ball-ball
...
This is amazing! Why, again? Because we need to calculate every single possible
next event, and a subset of those possible next events are **all ball-ball
collisions**. And this handy equation tells us if a ball-ball collision occurs
(via the existence of real, positive root) and if it occurs, it also tells us
when it occurs (the smallest, real, positive root). So by solving for the roots
of Eq. $\eqref{ball_poly}$ for each and every ball pair, we calculate the event
times for all ball-ball collisions.

#### --- Ball-cushion collision times

Another subset of possible events are all of the ball-cushion collisions---and
there are a lot of collisions to account for. In my model, I deconstruct the
...
introduced that depends on how airborne the ball is, which is depicted in Figure 13. Neverthless,
...
[(Source)](https://en.wikipedia.org/wiki/Distance_from_a_point_to_a_line). Rather than use an arbitrary point $(p_x, p_y, p_z)$, here I throw in
...
treated as a line that coincides with the cushion segments's edge, depicted in orange in Figure 16.
...
with a collision plane (unlike the scenario in Figure 13).
...
cushion, $h$. I assume here that $h$ is constant, but it may be fun
...
roots of which describe the time until collision of ball $i$ and cushion segment $j$. The only thing that
...
In comparison to the ball-cushion collision, this is a breeze. The ball-slate collision occurs either when a sliding ball has a $-z-$velocity component, or an airborne ball hits the slate. The first is somewhat pendantic, because if it has a $-z-$velocity component, the time until the ball-slate collision is by definition 0. So the real calculation is determining when an airborne ball hits the slate.
...
Actually, I'd like to modify the above statement slightly. If you absolutely _slam_ a ball into a pocket, it is likely to be slightly airborne, if only by a millimetre. Yet it still falls into the pocket because the back lip of the pocket is at a height higher than the ball's radius, thus redirecting the ball _into_ the pocket. Let's call this lip height $h_M$ and assume that any ball with $r_z(t)$ less than $h_M$ is pocketed, and any ball greater than $h_M$ isn't. An obviously crude means of modelling what in reality is a complex rigid body dynamics problem that doesn't easily lend itself to continuous event-based evolution.

Based on my own table, it seems like a reasonable value for $h_M$ is $(7/5) R$.
...
### (4) Calculating \Delta t_E

FIXME

#### --- Number of possible events?

How many potential events are there to solve for? Well, this will depend on the number of pockets, cushions, balls, as well as the states the balls are in. But for the sake of example, let's consider a standard table with 6 pockets, and a standard set of 15 balls + the cue ball. As you will see later on, I decompose the cushion surface  into straight line segments, each of which must be tested for ball-cushion collisions independently. For a standard table with 6 pockets, there are 18 such segments (Figure FIXME). So the makeup of agents that are potentially involved in events is as follows:
- 16 balls
- 18 cushion segments
- 6 pockets
...
**How many potential transition events?**
...
**How many potential ball-ball events?**

If there are $N$ balls on the table, collisions between each combination must be checked. That means there are $N (N-1) / 2$ potential ball-ball collisions to check. **For 16 balls thats 120 potential events.**
...
**How many potential ball-cushion events?**

With 18 cushion segments and 16 balls, each ball must be tested against each cushion segment explicitly. **That's $18x16=288$ potential ball-cushion events**. This one really hurts, but decomposing the cushion surface into linear segments is a necessary evil.
...
**How many potential ball-slate events?**

Ball-slate collision events are relatively rare. They are only relevant when a ball is airborne and/or has a velocity component into the table. So calculating a ball-slate collision is only necessary under these circumstances, which is going to be a pretty rare occurence, requiring explicit calculation perhaps just a handful of times throughout the entire evolution of a shot. Just to associate a number, let's esimate that **we have ~1 potential transition event**.

**How many potential ball-pocket events?**

With 16 balls and 6 pockets, each ball must be tested against each pocket explicitly. **That means $16x6=96$ potential ball-pocket events**.
...
I got so far into the weeds that even I need a summary of what the hell I'm doing:
...
FIXME some other stuff

### (6) A real example

FIXME Show network of a real shot

## Closing remarks

Maybe do a complexity analysis

Setup expectations for the next post

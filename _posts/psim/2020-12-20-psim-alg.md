...
event occurs? As it turns out, the most feasible strategy is to explicitly calculate **every
possible next event**. If you calculate the time until **literally every possible next event**,
then naturally the one that occurs in the shortest amount of time is the one that physically occurs.
...
In what follows, I gruesomely go through the algebra for
calculating every single kind of event time.
...
_**Figure 10**. On overhead view of a collision trajectory of the $i$th and $j$th balls. The magnitude of the distance
...
**The collision condition** for a collision of the $i$th ball with the $j$th
cushion segment occurs when the $i$th ball contacts the $j$th cushion segment's
...
connecting $\vec{p}_1^{(j)}$ and $\vec{p}_2^{(j)}$.

The shortest distance between a point $(p_x, p_y, p_z)$ and the collision plane is defined by

$$
d = \frac{
    \lvert
        (p_{2x}^{(j)}-p_{1x}^{(j)})(p_{1y}^{(j)}-p_y) - (p_{1x}^{(j)}-p_x)(p_{2y}^{(j)}-p_{1y}^{(j)})
    \rvert
}{
    \sqrt{
        (p_{2x}^{(j)}-p_{1x}^{(j)})^2 + (p_{2y}^{(j)}-p_{1y}^{(j)})^2
    }
}
\label{dist_to_plane}
$$
...
the trajectory of the $i$th ball from Eq. $\eqref{quad_r}$:

$$
d_{ij}(t) = \frac{
    \lvert
        (p_{2x}^{(j)}-p_{1x}^{(j)})(p_{1y}^{(j)}-r_y^{(i)}(t)) - (p_{1x}^{(j)}-r_x^{(i)}(t))(p_{2y}^{(j)}-p_{1y}^{(j)})
    \rvert
}{
    \sqrt{
        (p_{2x}^{(j)}-p_{1x}^{(j)})^2 + (p_{2y}^{(j)}-p_{1y}^{(j)})^2
...
d_{ij}(t) = \frac{
    \lvert
        (p_{2x}^{(j)}-p_{1x}^{(j)})
        (p_{1y}^{(j)}-(a_y^{(i)} t^2 + b_y^{(i)} t + c_y^{(i)})) - \\
        (p_{1x}^{(j)}-(a_x^{(i)} t^2 + b_x^{(i)} t + c_x^{(i)}))
        (p_{2y}^{(j)}-p_{1y}^{(j)})
    \rvert
}{
    \sqrt{
...
where $d_{ij}$ is the distance between the $i$th ball and the $j$th cushion segment.

[![ball_cushion_time]({{images}}/ball_cushion_time.jpg)]({{images}}/ball_cushion_time.jpg){:.center-img .width-70}

_**Figure 14**. A top down view of a collision trajectory of the $i$th ball with the $j$th cushion segment.
...
With an expression that describes the distance between the $i$th ball and the $j$th cushion segment, I can
...
So this is the collision condition, which is quadratic with respect to time. **Calculating the
...
- solve polynomial to get dt_E
- calculate the position of the ball at t + dt_E
- solve for s
- 0 < s < 1?
...
The alleged collision detected by the collision condition that occurs between the ball and cushion segment 2 is illegitimate because the contact point lies outside the domain defined between $(p _{1x}^{(2)}, p _{1y}^{(2)})$ and $(p _{2x}^{(2)}, p _{2y}^{(2)})$ (quantified by $s^{(2)} > 1)$. On the other hand, the collision between the ball and cushion segment 1 is legitimate because it occurs within $(p _{1x}^{(1)}, p _{1y}^{(1)})$ and $(p _{2x}^{(1)}, p _{2y}^{(1)})$, which happens when $0 \le s^{(1)} \le 1$.




---however its not quite accurate, because it
marks when a ball collides with a plane that extends infinitely in the
$xy-$plane, whereas the collision plane terminates at the points
$\vec{p}_1^{(j)}$ and $\vec{p}_2^{(j)}$. An example is detailed in Figure 15:

but there is just one
catch. This collision condition does not respect the fact that the collision
plane does not extend infinitely along the $xy-$plane, but terminates at the
points $p

<div class="extra-info" markdown="1">
<span class="extra-info-header">Time until ball-cushion collision event (I)</span>

Let $\Delta t_E$ be the time until collision between the $i$th ball and $j$th cushion segment,
...
If no real positive root exists, the $i$th ball and $j$th cushion segment do not collide.
...
the distance of the $i$th ball to the $j$th cushion segment as a function of time.
...
Let $\Delta t_E$ be the time until collision between the $i$th ball and $j$th cushion segment, where the
